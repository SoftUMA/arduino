...
namespace user__ {
	#include <math.h>
	const char* SALIDA = "bin/output/output15.plt";
	...
	void loop_relay(unsigned long curr_ms, float tmp_val, struct Relay& relay) {
		if (curr_ms - relay.last_ms >= RELAY_PERIOD) {
			float epsilon = DESIRED_TEMP - tmp_val;
			relay.last_ms += RELAY_PERIOD;
			
			if (epsilon <= 0) {
				relay.status = LOW;
				relay.duty_cycle = 0 * RELAY_PERIOD / 100;
			} else if (epsilon > 0 && epsilon < 5) {
				relay.status = HIGH;
				relay.duty_cycle = (20 * epsilon) * RELAY_PERIOD / 100;
			} else {
				relay.status = HIGH;
	...
	void setup_monitor(unsigned long curr_ms, struct Monitor& mtr) {
		...
		if (Serial) {
			Serial.println("#> SIMULACION Arduino Control de Temperatura Ciclo Trabajo (5000ms)");
